...
- **Frontend**: Vite build → static files → GCS bucket (static website hosting) → fronted by Cloudflare (DNS + TLS/CDN) at `app.<subdomain>`.
- **Backend**: Docker image → Artifact Registry → Cloud Run service → fronted by Cloudflare at `api.<subdomain>` (or a single subdomain path-split, see `infra.md`).
